...
| Maps | `client/src/components/ArmeniaMap.tsx` is a real, location-based map — **Leaflet, keyless** (the old deterministic SVG atlas was replaced on explicit request), using **standard OpenStreetMap tiles** muted into a brand surface via a CSS filter on `.leaflet-tile-pane` (index.css), plus **leaflet.markercluster** to merge nearby listings into a brand count pill. (Note: CARTO's basemaps were tried but now watermark "API KEY REQUIRED" for keyless use — don't reintroduce a hosted styled basemap that needs a key; mute plain OSM with CSS instead.) It's location-aware: `single`/detail listings center on their coordinates (street zoom for Yerevan, wider elsewhere), multi-listing views fit to all pins. Keep its public props (`listings`/`selectedId`/`onSelect`/`single`/`className`) stable — every discovery surface depends on them. OSM tiles are the one sanctioned external image source (the "Assets" rule's no-external-hosts applies to listing/brand imagery, not this explicitly-chosen map); keep the OSM attribution control. Don't swap to a keyed provider (Google/CARTO) without asking — the point here is zero key/billing. |
