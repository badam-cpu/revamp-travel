...
| `TWILIO_ACCOUNT_SID` / `TWILIO_AUTH_TOKEN` / `TWILIO_SMS_FROM` / `TWILIO_WHATSAPP_FROM` | Enable **SMS + WhatsApp** booking notifications to the customer on top of email (`server/sms.ts`, dispatched via `server/notify.ts`). `TWILIO_SMS_FROM` is an E.164 sender (`+374…`) or a Messaging Service SID (`MG…`); `TWILIO_WHATSAPP_FROM` is your WhatsApp sender (sandbox `+14155238886` or an approved number). **Without them, SMS/WhatsApp silently no-op** — email + the unified-inbox mirror still work. **WhatsApp caveat:** business-initiated messages outside the 24-hour customer-service window require **pre-approved Twilio Content templates**; the plain-text body only delivers inside that window or the Twilio sandbox. Each channel is independent, so you can enable SMS without WhatsApp or vice versa. | No (recommended for production). | none |
