...
function HeroCarousel({ photos, title, onOpen, badge }: { photos: string[]; title: string; onOpen: (i: number) => void; badge?: ReactNode }) {
  ...
  return (
    <div
      className="group relative h-[300px] overflow-hidden rounded-[14px] bg-basalt/5 sm:h-[400px] lg:h-[460px]"
...
export default function ListingPage({ params }: { params: { slug: string } }) {
  ...
  const factVal = (label: string) => listing.facts?.find((f) => f.label.toLowerCase() === label.toLowerCase())?.value?.trim();
