...
export function BrandMark({ light = false, compact = false }: { light?: boolean; compact?: boolean }) {
  if (compact) {
    return (
      <Link href="/" aria-label="Revamp home" className={cn("grid h-11 w-11 place-items-center rounded-[13px] text-[1.15rem] font-bold tracking-[-0.07em]", light ? "bg-white text-[#212121]" : "bg-[#F15822] text-white")}>
        re.
      </Link>
    );
  }
