...
async function searchLocation(query: string, latLng?: string): Promise<string | null> {
  ...
  const res = await fetch(`${BASE}/location/search?${params.toString()}`, { headers: headers() });
  const json = (await res.json().catch(() => ({}))) as { data?: SearchRow[]; message?: string; error?: { message?: string } };
  if (!res.ok) throw new Error(json.error?.message || json.message || `Tripadvisor search HTTP ${res.status}.`);
...
async function locationDetails(locationId: string): Promise<TripadvisorMatch | null> {
  const k = key();
  if (!k) throw new Error("No Tripadvisor API key is configured.");
  const res = await fetch(`${BASE}/location/${encodeURIComponent(locationId)}/details?key=${encodeURIComponent(k)}&language=en`, { headers: headers() });
  ...
  if (!res.ok || !json.location_id) throw new Error(json.error?.message || json.message || `Tripadvisor details HTTP ${res.status}.`);
